...
class TestAddToCart:
    ...
    @pytest.mark.asyncio
    async def test_add_to_cart_success(self, login_user):
        ...
        async with AsyncClient(
            ...
            for item in cart_items["items"]:
                assert str(added_item["id"]) in item["id"]

    @pytest.mark.asyncio
    async def test_add_to_cart_unavaliable_size(self, login_user):
        '''Test add_to_cart Success'''
        async with AsyncClient(
            transport=ASGITransport(app=app),
            base_url="http://test"
            ...
            assert add_to_cart_res.status_code == 400
            assert "detail" in add_to_cart_res.json()
            assert add_to_cart_res.json(
            )["detail"] == "Size 8 is not available for this product"

    @pytest.mark.asyncio
    async def test_add_to_cart_not_enough_stock(self, login_user):
        '''Test add_to_cart Success'''
        async with AsyncClient(
            transport=ASGITransport(app=app),
            base_url="http://test"
            ...
            assert "detail" in add_to_cart_res.json()
            assert add_to_cart_res.json(
            )["detail"] == "Insufficient stock: available stock for size 10 is 10"

    @pytest.mark.asyncio
    async def test_add_to_cart_invalid_size(self, login_user):
        '''Test add_to_cart Success'''
        async with AsyncClient(
            transport=ASGITransport(app=app),
            base_url="http://test"
            ...
            assert add_to_cart_res.status_code == 422
            assert "size" in add_to_cart_res.json()["detail"][0]["loc"]

    @pytest.mark.asyncio
    async def test_add_to_cart_product_exists(self, login_user):
        '''Test add_to_cart Success'''
        async with AsyncClient(
            transport=ASGITransport(app=app),
            base_url="http://test"
            ...
            add_to_cart_res_2 = await client.post(
                "/api/cart/add",
                headers=auth_headers,
                json={
                    "product_id": product_id,
            ...
            assert add_to_cart_res_2.status_code == 201

            cart_items_res = await client.get(
                "/api/cart/",
                headers=auth_headers,
                follow_redirects=True
            )
            cart_items = cart_items_res.json()
